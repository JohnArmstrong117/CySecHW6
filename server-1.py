...
# Initialize the database: create the "students" table and refresh CPA key for each student
def init_db():
    ...
    """)
    # Read data from the JSON file and initialize (only import records with role "student")
        ...
        with open(USER_FILE, "r") as f:
            ...
            for user in users:
                if user.get("role") == "student":
                    ...
                    row = cursor.fetchone()
                    if row:
                        # Refresh CPA key for existing student
                        cursor.execute("UPDATE students SET cpa_key = ? WHERE username = ?", (new_cpa_key, username))
                    else:
                        # Insert new student with initial score 0
                        cursor.execute("""
